module bank::bank {
  ...
  use bank::sui_dollar::{Self, CapWrapper, SUI_DOLLAR};  
  ...
  public fun swap_sui(cap:&mut CapWrapper, self: &mut Bank, coin_in: Coin<SUI>, ctx: &mut TxContext): Coin<SUI_DOLLAR> {
    let swap_value = coin::value(&coin_in);
    let sui_dollar_value = (((swap_value as u128) * EXCHANGE_RATE / 100) as u64);
    
    let sui_dollar_swap = sui_dollar::mint(cap, sui_dollar_value, ctx);
  ...
  public fun swap_sui_dollar(cap:&mut CapWrapper, self: &mut Bank, coin_in: Coin<SUI_DOLLAR>, ctx: &mut TxContext): Coin<SUI> {
    ...
    let sui_value = (((swap_value as u128) * EXCHANGE_RATE / 100) as u64);    

    sui_dollar::burn(cap, coin_in);

    let bank_balance = balance::value(&self.balance);

    assert!(bank_balance >= sui_value, ENotEnoughBalance);

    coin::from_balance(balance::split(&mut self.balance, sui_value), ctx)
